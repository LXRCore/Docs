...
### 2.3 lxr-creator — **NOT yet on the bridge** (headers + banner only)
* Done: CREATOR word art in `config.lua, config_traits.lua, fxmanifest.lua, client/main.lua, server/main.lua, server/traits.lua`; blood-red card at the end of `server/main.lua` (inline, not Bridge).
* Still calls lxr-core directly (~50 sites): `server/main.lua` (`RateLimit`, `Log`, `Notify`, `Config.Player.maxCharacters`, `Player.GetCharacters/ValidateCitizenId/Login/Logout/DeleteCharacter`, `Shared.StarterItems`, `Callback.Register('lxr-creator:server:characters' | ':appearance')`, `Brand`, `Commands.Add('logout' | 'closemulti')`), `server/spawn.lua` (`RateLimit`, `DB.Scalar players.last_updated`, `Emit('lxr:spawn:chosen')`, `GetPlayer`), `server/traits.lua` (`Config.Player.skills/maxSlots/maxWeight/xpPerLevel`, `SetPlayerData('slots'|'weight')`, commands `traits/resettraits/retrait`), `client/main.lua` (`Callback.Await`, `Brand`), `client/spawn.lua` (`Notify`, `LXR.Player.Spawned()` → `lxr:client:loaded` + `lxr:player:spawn`).
* **The plan** (docs/BRIDGE-PLAN-CHARACTER-SUITE.md §4, researched from the reference code):
  * **RSG**: select screen reads characters from `players` by licence (`Bridge.Characters` / the core's `GetPlayersData`), create → `RSGCore.Player.Login(src, false, newData)` with `newData.cid` + `charinfo`, select → `Player.Login(src, citizenid)`, delete → `RSGCore.Player.DeleteCharacter(src, citizenid)`, skin saved to **our** table through lxr-clothing `SaveAppearance` (mirror to `playerskins` optional), then the client fires **`RSGCore:Client:OnPlayerLoaded`** and the server `RSGCore:Server:PlayerLoaded` fires from Login. `rsg-multicharacter`, `rsg-appearance`, `rsg-spawn` are then not ensured. Starter items via rsg-inventory.
  * **VORP**: create → `Core.getUser(src).addCharacter({ firstname, lastname, skin = '{}', comps = '{}', compTints = '{}', age, gender, charDescription, nickname })` (the core inserts `characters` and sets it used), select → `user.setUsedCharacter(charid)`, delete → `user.removeCharacter(charid)`, then `TriggerClientEvent('vorp:initCharacter', src, coords, heading, false)` and everything wakes on `vorp:SelectedCharacter`; `vorp_character` not ensured. Later skin writes: keep ours in `lxr_appearance` (VORP's `skinPlayer/compPlayer` stay empty or mirrored via `M.Export('vorp')`).
  * **standalone**: own `lxr_characters (id, license, slot, firstname, lastname, birthdate, gender, nationality, created, last_played)` table + select screen; identity = row id; no money/items.
  * Traits: `Config.TraitFlow.skills.list` must have a config default (today falls back to `LXRCore.Config.Player.skills`); slots/weight perks only on frameworks with an inventory that exposes them (LXR/RSG `SetPlayerData`), no-op on VORP.
  * Add `Bridge.Login/Logout/Create/Delete/Select` to the adapters **only in the creator's bridge copy** (or a `bridge/characters.lua` next to the shared files) so clothing/barber's bridge stays identical.
* Existing creator docs: `docs/API.md, ARCHITECTURE.md, INSTALL.md, TRAITS.md` — add FRAMEWORKS / CONFIG / PERMISSIONS / ESCROW / TROUBLESHOOTING / MIGRATION (characters) in the playerstore layout once bridged.
...
2. **RSGCore-Dev profile**: `ensure oxmysql, rsg-core, rsg-inventory, ox_lib, lxr-clothing, lxr-barber` (keep rsg-multicharacter/rsg-appearance for now — the creator is not bridged). Card: framework `RSG Core (rsg-core)`, tables `lxr_appearance / lxr_outfits`. Run `lxr-clothing:migrate rsg dry` against rsg-appearance's `playerskins`, then real; log in → the imported look must show on `RSGCore:Client:OnPlayerLoaded`. Tailor pay via `RemoveMoney`, kits via `SetItemData`.
3. **VORPCore profile**: `ensure oxmysql, vorp_core, vorp_inventory, lxr-clothing, lxr-barber`; migrate vorp dry/real; `vorp:SelectedCharacter` applies the look; TipRight notifications; `getItemByName`/`setItemMetadata` for kits.
...
1. **lxr-creator on the bridge** (RSG Login/Delete flow, VORP addCharacter/initCharacter/SelectedCharacter flow, standalone `lxr_characters`), then its docs set — the biggest block (est. 1½ days in the plan).
2. In-game test pass (§4) on the three profiles; fix what breaks.
