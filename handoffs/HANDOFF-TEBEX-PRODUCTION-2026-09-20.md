...
5. **Release gate before every release**: `python tools/release_gate.py lxr-clothing lxr-barber lxr-creator --bridge-of lxr-clothing` (68 checks: the five checkers, offline tests, manifest, escrow set, name guard in every server/client file, no framework calls in the logic, secret scan, locales, docs set, CHANGELOG entry, bridge byte-parity, LF/BOM/tabs/size). `lxr-character-plus/tools/pack.py` refuses to build zips unless it passes. **Five checkers before every commit** (from the workspace root): `python tools/native_check.py <repos>`, `lua_check.py`, `kit_check.py`, `js_check.py`, `perf_check.py`. Plus `lua tests/run.lua` inside repos that have `tests/` (clothing 10 tests, barber 3). **No test sections in READMEs** (keep `tests/`, do not advertise).
...
7b. **Bridge RPC channel** is throttled per player (40 / 5 s) and type-checked; unknown names are logged. Handlers keep their own limits.
